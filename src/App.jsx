    ...
    <div>
      ...
      {showAchat&&(
        <div style={{background:"#eff6ff",borderRadius:14,padding:16,marginBottom:16,border:"1.5px solid #bfdbfe",animation:"up .2s cubic-bezier(.22,1,.36,1)"}}>
          ...
          <input type="number" min="1" value={achatAmt} onChange={e=>setAchatAmt(e.target.value.replace(/^-/,""))}
    ...
    <div dir={rtl?"rtl":"ltr"} style={{minHeight:"100svh",background:"#f1f5f9",fontFamily:"system-ui,-apple-system,sans-serif",display:"flex"}}>
      ...
      <div style={{flex:1,padding:"28px 32px",paddingBottom:40,overflowY:"auto",maxWidth:900,width:"100%",margin:"0 auto",boxSizing:"border-box"}}>
        ...
        {tab==="history"&&(()=>{
          ...
          const exportCSV=()=>{
            const rows=[
              ["Date","Type","Description","Client","Montant","Statut","Facture"],
              ...filtered.map(tx=>[
                tx.date, tx.type==="income"?"Revenu":"Dépense",
                tx.desc||"", tx.client||"",
                tx.amount, tx.paid?"Payé":"En attente",
                tx.invoiceId||""
              ])
            ];
            const csv=rows.map(r=>r.map(c=>`"${String(c).replace(/"/g,'""')}"`).join(",")).join("\n");
              ...
              <div style={S.card({marginBottom:12})}>
                ...
                <div style={{position:"relative",marginBottom:12}}>
                  ...
                  <input value={histSearch} onChange={e=>setHistSearch(e.target.value)}
                    placeholder="Rechercher..." style={{...S.inp({paddingLeft:36})}}/>
                </div>
                {/* Type filter */}
                <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap"}}>
                  {[["all","Tout"],["income","↑ Revenus"],["expense","↓ Dépenses"]].map(([k,l])=>(
                    <button key={k} onClick={()=>setHistFilter(k)}
                      style={{padding:"6px 14px",borderRadius:20,border:"none",cursor:"pointer",fontSize:12,fontWeight:700,
                        background:histFilter===k?"#2563EB":"#f3f4f6",color:histFilter===k?"#fff":"#6b7280"}}>
                      {l}
                    </button>
                  ))}
                  <button onClick={exportCSV}
                    style={{marginLeft:"auto",padding:"6px 14px",background:"#059669",color:"#fff",border:"none",borderRadius:20,fontSize:12,fontWeight:700,cursor:"pointer"}}>
                    📊 Export CSV
                  </button>
                </div>
                {/* Date range */}
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  <div>
                    <div style={{fontSize:10,color:"#9ca3af",marginBottom:4,fontWeight:600}}>Du</div>
                    <input type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)} style={S.inp({fontSize:13})}/>
                  </div>
                  <div>
                    <div style={{fontSize:10,color:"#9ca3af",marginBottom:4,fontWeight:600}}>Au</div>
                    <input type="date" value={dateTo} onChange={e=>setDateTo(e.target.value)} style={S.inp({fontSize:13})}/>
                  </div>
                </div>
              ...
              {filtered.length>0&&(
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:12}}>
                  <div style={{background:"#ecfdf5",borderRadius:10,padding:"10px",textAlign:"center"}}>
                    <div style={{fontWeight:900,fontSize:14,color:"#059669"}}>{fmt(totalIncome,lang)}</div>
                    <div style={{fontSize:9,color:"#065f46",fontWeight:700,marginTop:2}}>REVENUS</div>
                  </div>
                  <div style={{background:"#fef2f2",borderRadius:10,padding:"10px",textAlign:"center"}}>
                    <div style={{fontWeight:900,fontSize:14,color:"#dc2626"}}>{fmt(totalExpense,lang)}</div>
                    <div style={{fontSize:9,color:"#b91c1c",fontWeight:700,marginTop:2}}>DÉPENSES</div>
                  </div>
                  <div style={{background:"#eff6ff",borderRadius:10,padding:"10px",textAlign:"center"}}>
                    <div style={{fontWeight:900,fontSize:14,color:"#2563EB"}}>{filtered.length}</div>
                    <div style={{fontSize:9,color:"#1d4ed8",fontWeight:700,marginTop:2}}>ENTRÉES</div>
                  </div>
                </div>
              )}
        ...
        {tab==="rapport"&&(()=>{
          ...
          // ديون قديمة (أكثر من 30 يوم)
          const today30=new Date(); today30.setDate(today30.getDate()-30);
          const oldDebts=invoices.filter(inv=>{
          ...
          const exportRapport=()=>{
            const rows=[
              [`Rapport ${months[selMonth]} ${selYear}`],
              [],
              ["Indicateur","Montant"],
              ["Revenus encaissés",revenus],
              ["Dépenses",depenses],
              ["Bénéfice net",benefice],
              ["À encaisser",attente],
              [],
              ["Factures émises",nbInv],
              ["Factures payées",nbPaid],
              ["Factures impayées",nbInv-nbPaid],
            ];
            const csv=rows.map(r=>r.map(c=>`"${String(c).replace(/"/g,'""')}"`).join(",")).join("\n");
            const blob=new Blob(["\uFEFF"+csv],{type:"text/csv;charset=utf-8"});
            const url=URL.createObjectURL(blob);
            const a=document.createElement("a");
            a.href=url; a.download=`rapport_${prefix}.csv`;
            a.click(); URL.revokeObjectURL(url);
          };
            ...
            <div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
                <button onClick={()=>{if(selMonth===0){setSelMonth(11);setSelYear(y=>y-1);}else setSelMonth(m=>m-1);}}
                  style={{width:36,height:36,borderRadius:10,border:"1.5px solid #e5e7eb",background:"#fff",cursor:"pointer",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center"}}>‹</button>
                <div style={{flex:1,textAlign:"center",fontWeight:900,fontSize:18,color:"#111"}}>{months[selMonth]} {selYear}</div>
                <button onClick={()=>{if(selMonth===11){setSelMonth(0);setSelYear(y=>y+1);}else setSelMonth(m=>m+1);}}
                  style={{width:36,height:36,borderRadius:10,border:"1.5px solid #e5e7eb",background:"#fff",cursor:"pointer",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center"}}>›</button>
                <button onClick={exportRapport}
                  style={{padding:"8px 14px",background:"#059669",color:"#fff",border:"none",borderRadius:10,fontSize:12,fontWeight:700,cursor:"pointer"}}>
                  📊 Export
                </button>
              </div>
              ...
              {/* Stats factures */}
              <div style={S.card({marginBottom:16})}>
                <div style={{fontSize:12,fontWeight:700,color:"#374151",marginBottom:12}}>📊 Factures — {months[selMonth]}</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8}}>
                  {[
                    {l:"Émises",v:nbInv,c:"#374151"},
                    {l:"Payées",v:nbPaid,c:"#059669"},
                    {l:"Impayées",v:nbInv-nbPaid,c:"#dc2626"},
                    {l:"À encaisser",v:fmt(attente,lang),c:"#d97706"},
                  ].map(s=>(
                    <div key={s.l} style={{textAlign:"center",padding:"10px 6px",background:"#f9fafb",borderRadius:10}}>
                      <div style={{fontWeight:900,fontSize:16,color:s.c}}>{s.v}</div>
                      <div style={{fontSize:9,color:"#6b7280",fontWeight:700,marginTop:2}}>{s.l}</div>
              ...
              {oldDebts.length>0&&(
                <div style={{background:"#fef2f2",borderRadius:14,padding:16,border:"1.5px solid #fecaca",marginBottom:16}}>
                  <div style={{fontWeight:800,fontSize:14,color:"#b91c1c",marginBottom:12}}>
                    ⏰ Factures impayées depuis + 30 jours ({oldDebts.length})
                  </div>
                  {oldDebts.map(inv=>{
                    const days=Math.floor((new Date()-new Date(inv.date))/(1000*60*60*24));
                    const reste=inv.total-(inv.paidAmount||0);
              ...
              {mInvs.length>0&&(()=>{
                ...
                return(
                  <div style={S.card()}>
                    <div style={{fontSize:12,fontWeight:700,color:"#374151",marginBottom:12}}>🏆 Top clients — {months[selMonth]}</div>
                    {top.map(([name,total],i)=>(
                      <div key={name} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:i<top.length-1?"1px solid #f3f4f6":"none"}}>
                        <div style={{width:24,height:24,borderRadius:8,background:["#fbbf24","#94a3b8","#c2853a","#6366f1","#10b981"][i],display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:900,color:"#fff",flexShrink:0}}>{i+1}</div>
                        <div style={{flex:1,fontWeight:600,fontSize:13,color:"#111"}}>{name}</div>
